...
function generateNewAnswers() {
  ...
  let copy = [];
  //for each object in clickableTextObjects, if it is not an answer, add it to copy
  clickableTextObjects.forEach(x => {
    if (!x.isAnAnswer) {
      copy.push(x);
    }
  });
  //clear clickableTextObjects but can't reassign because const
  clickableTextObjects.length = 0;
  //add the copy back to clickableTextObjects
  copy.forEach(x => clickableTextObjects.push(x));
